...
const KDO = {
	...
	use(fn, myName) {
		...
		if (typeof fn === 'object') {
			...
			this.chain.push(fn.length === 2 ? fn : autoNext(fn));
		}
	},

	async __kdoNext(args) {
		...
		// Bind "this" to this.__kdoNext to make it to points to the correct object (the instance of KDO).
		return await fn(this.args, this.__kdoNext.bind(this));
	},

	async do() {
		tree.level(+1);
		const result = await this.__kdoNext();
		tree.level(-1);

		tree.printDone();
		return this.__kdoGetResult(result);
	},
